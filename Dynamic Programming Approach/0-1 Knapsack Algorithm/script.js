...
function animate(n, maxw, w) {
  ...
  for (let j = 0; j < maxw + 1; j++) {
    let cell = table.getElementsByTagName("tr")[0].getElementsByTagName("td")[
      j
    ];
    cell.classList.remove("hide");
  }
  for (let i = 1; i < n + 1; i++) {
    for (let j = 1; j < maxw + 1; j++) {
      if (w[i - 1] <= j) {
        let currentCell = table
          .getElementsByTagName("tr")
          [i].getElementsByTagName("td")[j];
        let cell1 = table
          .getElementsByTagName("tr")
          [i - 1].getElementsByTagName("td")[j];
        let cell2 = table
          .getElementsByTagName("tr")
          [i - 1].getElementsByTagName("td")[j - w[i - 1]];
        setTimeout(() => {
